...
* There is a `Record Dream Report` button that will start to record from whatever external recording device is selected in the **Dream Recording** window's `Device` dropdown. It can also pop open a survey URL — I use this to open a dream report survey I have set up on Qualtrics. Add your surveys with the `Manage…` button next to the survey dropdown (each has a name and URL, saved to your settings YAML); pick one to open automatically when recording starts, or open any saved survey on its own from `File > Surveys`. If planning to record dreams, choose the input device in that same `Device` dropdown.

* Each routing window (audio cue, noise, intercom, dream recording, visual) picks its own device, and those selections are saved in the `.smacc` settings file, so your rig's setup is restored on the next launch. If a saved device isn't connected when the file loads, SMACC keeps the default and tells you which one is missing. Plug in a device after SMACC is already open — including a BlinkStick — and pick up `File > Refresh devices` (or press `F5`) to rescan without restarting (audio devices rescan only while nothing is playing or recording).
